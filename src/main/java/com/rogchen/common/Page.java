...
public class Page<T> {
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
        ...
        if (StringUtils.isBlank(no)) {
            ...
            if (StringUtils.isNotBlank(pageIndex) && StringUtils.isNumeric(pageIndex)) {
                no_ = Integer.parseInt(pageIndex) + 1;
            }
        } else {
            no_ = Integer.parseInt(no);
        }

        //if (StringUtils.isNumeric(no)){
        if (no_ > 0) {
            //CookieUtil.addCookie(response, "pageNo", no);
            CookieUtil.addCookie(response, "pageNo", no_ +"");
            this.setPageNo(no_);
        } else if (request.getParameter("repage") != null) {
            no = CookieUtil.getCookieValue(request, "pageNo");
            if (StringUtils.isNumeric(no)) {
		...
		if (StringUtils.isNumeric(size)){
			...
			CookieUtil.addCookie(response, "limit", size);
			this.setPageSize(Integer.parseInt(size));
		}else if (request.getParameter("repage")!=null){
			no = CookieUtil.getCookieValue(request, "pageSize");
			if (StringUtils.isNumeric(size)){
				this.setPageSize(Integer.parseInt(size));
			}
	 ...
	 * @param pageSize 分页大小
	 * @param count 数据条数
	 * @param content 本页数据对象列表
	 */
	public Page(int pageNo, int pageSize, long count, List<T> content) {
		this.setCount(count);
...
//	}
	...
	public int getFirstResult(){
		...
		if (firstResult >= getCount()) {
			firstResult = Integer.parseInt(getCount()+"");
		}
		return firstResult;
	}
	/**
